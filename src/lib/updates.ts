...
const postsDirectory = path.join(process.cwd(), 'public/updates'); // process.cwd() returns the absolute path of the current working directory
...
export async function getSortedUpdatesData() {
  ...
  const allPostsDataPromise = fileNames.map(async (filename) => {
  ...
  const allPostsData = await Promise.all(allPostsDataPromise);
